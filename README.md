...
<img src="docs/showcase-hero.png" alt="A commerce schema drawn as a relationship map in a custom dark theme">

<sup>One schema — the demo `Commerce` model — in a custom "Atlas" theme, composed headless by Mappa
itself (`MappaShowcaseTest`), like every image on this page: the code shown is exactly what rendered the
map next to it, regenerated on every build so they can't drift apart.</sup>
...
<img src="docs/readme-quickstart.png" width="640" alt="A three-table store schema laid out automatically">
...
`image(width, height)` to render headless (docs, reports, CI), like every image here.
...
<img src="docs/readme-controls.png" width="720" alt="The commerce schema with curated controls applied">
...
<img src="docs/showcase-layouts.png" alt="The commerce schema laid out four ways: layered, radial, force, grid">
...
```java
Mappa.view(map).edges(MappaEdges.DIRECTIONAL).component();
```

<img src="docs/readme-edges.png" width="720" alt="The schema with directional gradient edges">
...
<img src="docs/showcase-spotlight.png" width="760" alt="Clicking the orders box spotlights it — neighbours lit, the rest dimmed, flow along its relationships">
...
<img src="docs/showcase-themes.png" alt="The same schema in the light, dark, and a custom Atlas theme">
...
- **Move** — drag any box to reposition it; edges re-route live to follow.
...
| `MappaMap` | The immutable model — `Entity`, `Field`, `Relationship`, `EntityKind` — plus `focus`, `select`, `toBytes`, `write`. |
| `MappaView` | Fluent view builder ending in `component()` (live), `image(w, h)` (PNG), `toSvg`/`writeSvg`, or `toInteractiveHtml`/`writeInteractiveHtml`; carries host `action`s. |
...
`build/mappa-*.png` — the render-and-look gate behind every change — plus `MappaReadmeExamplesTest` and
`MappaShowcaseTest`, which regenerate every image in this README from the exact code shown above.
